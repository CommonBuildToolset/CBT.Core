...
namespace CBT.Core.Tasks
{
    ...
    public sealed class RestoreModules : MarshalByRefObject, ICancelableTask, IDisposable
    {
        ...
        public bool Execute()
        {
            ...
            using (Semaphore semaphore = new Semaphore(0, 1, semaphoreName, out releaseSemaphore))
            {
                try
                {
                    ...
                    ModulePropertyGenerator modulePropertyGenerator = new ModulePropertyGenerator(_log, PackagesPath, GetPackageRestoreData(), PackageConfig);
        ...
        private PackageRestoreData GetPackageRestoreData()
        {
            if (string.IsNullOrWhiteSpace(AssetsFlag) || !File.Exists(AssetsFlag))
            {
                _log.LogMessage(MessageImportance.Low, $"Package reference {AssetsFlag} not found.  Either you are not using PackageReference elements for your packages or you are using a version of nuget.exe prior to 4.x, or the cbtmodules.proj does not import CBT in some way.");
                return null;
            }
            return JsonConvert.DeserializeObject<PackageRestoreData>(File.ReadAllText(AssetsFlag));
        }
